...
final class Application {
        ...
        public static function init() {
                self::setAutoLibs();           							//初始化$_lib 变量
                require SYS_CORE_PATH.'/model.php';    					//核心模型和核心控制器，在以后自己的模型和控制器中来继承核心模型和控制器
                require SYS_CORE_PATH.'/controller.php';			    //核心模型和控制器中主要可以是一些通用的方法和必须的组建的加载，下面我们先来写核心控制器
        }
        /**
         * 创建应用
         * @access      public
        ...
        public static function run($config){
                ...
                self::init();
                self::autoload();      //自动加载/system/lib 目录下的类库,并实例化类
                self::$_lib['route']->setUrlType(self::$_config['route']['url_type']); //设置url的类型
                $url_array = self::$_lib['route']->getUrlArray();                      //将url转发成数组 array([app]=admin,[controller]=>index,[action]=>,[params]=>)
        ...
        public static function autoload(){
                foreach (self::$_lib as $key => $value){
                        ...
                        $lib = ucfirst($key);             /*$lib 为./lib 目录下类的类名*/
                        self::$_lib[$key] = new $lib;     /*实例化类*/
                }
                //初始化cache
        ...
        public static function newLib($class_name){
                ...
                if(file_exists($app_lib)){
                        ...
                        require ($sys_lib);
                        return self::$_lib["$class_name"] = new ucfirst($class_name);    //加载 system/lib 目录下的自定义类
                }else{
